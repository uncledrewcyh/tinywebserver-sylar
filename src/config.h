...
/**
 * @brief 类型转换模板类(F 源类型(from_typq), T 目标类型(to_type))
 */
...
public:
    ...
    std::string toString() override {
        try {
            ...
            PUDGE_LOG_ERROR(PUDGE_LOG_ROOT()) << "ConfigVar::toString exception "
    ...
    bool fromString(const std::string& val) override {
        try {
            ...
            PUDGE_LOG_ERROR(PUDGE_LOG_ROOT()) << "ConfigVar::fromString exception "
    ...
    /**
     * @brief 设置当前参数的值
     * @details 比如当日志器参数改变时，执行回调改变日志器的参数.
     */
...
private:
    RWMutexType m_mutex;
    T m_val;
    //参数变更回调函数组, uint64_t key,要求唯一，一般可以用hash
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != GetDatas().end()) {
            ...
            if(tmp) {
                PUDGE_LOG_INFO(PUDGE_LOG_ROOT()) << "Lookup name=" << name << " exists";
                return tmp;
            } else {
                PUDGE_LOG_ERROR(PUDGE_LOG_ROOT()) << "Lookup name=" << name << " exists but type not "
                        << TypeToName<T>() << " real_type=" << it->second->getTypeName()
                        << " " << it->second->toString();
                return nullptr;
            }
        }
        ///name 必须由小写字母 下划线 数字构成
        if(name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678")
                != std::string::npos) {
            PUDGE_LOG_ERROR(PUDGE_LOG_ROOT()) << "Lookup name invalid " << name;
