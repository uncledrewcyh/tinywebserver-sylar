...
std::string HttpRequest::getParam(const std::string& key, const std::string& def) {
    initQueryParam();
    /// m_parserParamFlag = 0x2 => 解析完uri的参数
    initBodyParam();
    /// m_parserParamFlag = 0x2 => 解析完body的参数
...
#define PARSE_PARAM(str, m, flag, trim) \
    ...
    do { \
        ...
        m.insert(std::make_pair(trim(str.substr(last, key - last)), \
                    pudge::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)))); \
        if(pos == std::string::npos) { \
            break; \
        } \
        ++pos; \
...
void HttpRequest::initCookies() {
    ...
    PARSE_PARAM(cookie, m_cookies, ';', pudge::StringUtil::Trim);
...
void HttpResponse::setCookie(const std::string& key, const std::string& val,
    ...
    ss << key << "=" << val;
    if(expired > 0) {
        ss << ";expires=" << pudge::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
    }
    if(!domain.empty()) {
        ss << ";domain=" << domain;
    }
...
std::ostream& HttpResponse::dump(std::ostream& os) const {
    ...
    ///判断是否为文件类型
    if (m_file_response) {
        os << "content-length: " << m_file_ba->getReadSize() << "\r\n\r\n";
    } else {
        if(!m_body.empty()) {
            os << "content-length: " << m_body.size() << "\r\n\r\n"
            << m_body;
